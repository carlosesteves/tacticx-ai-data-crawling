...
def run_coach_pipeline(session: Session, coach_id: int, context: PipelineContext, page: CoachPage = None):
    ...
    for tenure in CoachService.parse_tenures(page):        
        # if (tenure.coach_id, tenure.club_id, tenure.start_date) not in context.tenure_cache:            
        context.tenure_repo.save(tenure)
        # context.tenure_cache.add((tenure.coach_id, tenure.club_id, tenure.start_date))
