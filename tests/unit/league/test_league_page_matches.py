...
def test_get_match_ids_from_fixture():
    ...
    page = LeaguePageMatches(requests.session(), league_code="GB1", season_id=2024, html_content=html_content)

    match_ids = page.get_match_ids()

    assert isinstance(match_ids, set)
    assert all(isinstance(m, int) for m in match_ids)
...
def test_get_match_ids_empty_fixture():
    ...
    page = LeaguePageMatches(requests.session(), league_code="GB1", season_id=2024, html_content=empty_html)
